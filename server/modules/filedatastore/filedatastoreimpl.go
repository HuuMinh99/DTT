...
func (datastore *FileDatastoreImpl) GetJobsByInterfaces(offset int, count int, Interface string, startTime int64, endTime int64) ([]*model.Job, int) {
	datastore.lock.RLock()
	defer datastore.lock.RUnlock()
	JobsFilter := make([]*model.Job, 0)
	// filter by Time and Interface ThanhDK
	for _, job := range datastore.jobsById {
		if !job.CreateTime.IsZero() && (startTime != 0 && endTime != 0) {
			if Interface != "" {
				Timestamp := job.CreateTime.Unix()
				if Timestamp <= endTime && Timestamp >= startTime {
					if job.SensorId == Interface {
						JobsFilter = append(JobsFilter, job)
					}
				}
			} else {
				Timestamp := job.CreateTime.Unix()
	...
	// sort array ThanhDK
	sort.Slice(JobsFilter, func(i, j int) bool {
		return JobsFilter[i].Id < JobsFilter[j].Id
	})
	jobPaging := make([]*model.Job, 0)
	index := 0
	for _, job := range JobsFilter {
		index++
		if index >= offset {
			jobPaging = append(jobPaging, job)
			if len(jobPaging) >= count {
				break
			}
		}
	}
	return jobPaging, len(JobsFilter)
}
...
func (datastore *FileDatastoreImpl) DeleteJob(job *model.Job) error {
	return datastore.deleteJob(job)
}
...
func (datastore *FileDatastoreImpl) removeJob(job *model.Job) error {
	filename := fmt.Sprintf("%d.json", job.Id)
	folder := filepath.Join(datastore.jobDir, sanitize.Name(job.SensorId))
	log.WithFields(log.Fields{
		"id":       job.Id,
...
func (datastore *FileDatastoreImpl) GetPacketsByTime(jobId int, offset int, count int, startTime int64, endTime int64, search string) ([]*model.Packet, int, error) {
	var packets []*model.Packet
	var err error
	var totalPages int
	job := datastore.GetJob(jobId)
	if job != nil {
		if job.Status == model.JobStatusCompleted {
			packets, totalPages, err = packet.ParsePcapByTime(datastore.getStreamFilename(job), offset, count, startTime, endTime, search)
			if err != nil {
				log.WithError(err).WithField("jobId", job.Id).Warn("Failed to parse captured packets")
				err = nil
			}
